<?php
if(class_exists("Gc_MessageBox_Options_Group_Container_Renderer")){
    return;
}
class Gc_MessageBox_Options_Group_Container_Renderer extends Gc_MessageBox_Container_Renderer{
    ...
    protected function initialize_state_option(){
        $this->state_option =  Gc_MessageBox_CF::create_and_init("Option",array("namespace" => $this->namespace , "id" => $this->group_descriptor->get_id()."_group" , "default" => "open"));
   ...
   public function render_item($key,$description,$counter){
      if($description->has_renderer()){
        ...
        $cnt = $renderer = Gc_MessageBox_CF::create($renderer_name);
      ...
      switch($description->get_type()){
       ...
       public function render_type_default($description){
         $renderer = Gc_MessageBox_CF::create("Text_Renderer");
       $renderer->render($description);
       }

       public function render_type_number($description){
       $renderer = Gc_MessageBox_CF::create("Number_Renderer");
       $renderer->render($description);
       }

       public function render_type_textarea($description){
       $renderer = Gc_MessageBox_CF::create("Textarea_Renderer");
       $renderer->render($description);
       }

       public function render_type_checkbox($description){
        $renderer = Gc_MessageBox_CF::create("Checkbox_Renderer");
        $renderer->render($description);
    }

    public function render_type_radio($description){
      $renderer = Gc_MessageBox_CF::create("Radio_Renderer");
        $renderer->render($description);
    }

       public function render_type_select($description){
        $renderer = Gc_MessageBox_CF::create("Select_Renderer");
        $renderer->render($description);
    }

    public function render_type_color($description){
        $renderer = Gc_MessageBox_CF::create("Color_Renderer");
        $renderer->render($description);
    }
       public function render_type_onoff($description){
        $renderer = Gc_MessageBox_CF::create("Button_Onoff_Renderer");
        $renderer->render($description);
       }
       public function render_type_darklight($description){
        $renderer = Gc_MessageBox_CF::create("Button_Darkligth_Renderer");
        $renderer->render($description);
       }

    public function render_type_fonttype_select($description){
        $renderer = Gc_MessageBox_CF::create("Fonttype_Select_Rnderer");
        $renderer->render($description);
    }

    
       public function render_type_slider($description){
           $renderer = Gc_MessageBox_CF::create("Slider_Renderer");
        $renderer->render($description);
       }
    ...
    public function after_item_render($key,$description,$counter){
        if($this->group_descriptor->has_sub_group($description->get_id())) {
            ...
            if($container->has_renderer()) {
                ...
                $renderer_name = Gc_MessageBox_CF::get_class_name("Options_Subgroup_Container_Renderer");
            }
