<?php
if(class_exists("Gc_MessageBox_Container_Renderer")){
    return;
}
abstract class Gc_MessageBox_Container_Renderer extends Gc_MessageBox_Abstract_Renderer{
    ...
    public function on_render_option($event){
     ...
     public function render($gci) {
         ...
         foreach($items as $key => $desc){
             $event = new Gc_MessageBox_Event(array('key' => $key,'descriptor' => $desc,'group_decriptor' => $this->group_descriptor, 'namespace' => $this->namespace));
