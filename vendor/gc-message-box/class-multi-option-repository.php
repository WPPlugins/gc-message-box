<?php
if(class_exists('Gc_MessageBox_Multi_Option_Repository')) {
    return;
}
class Gc_MessageBox_Multi_Option_Repository extends Gc_MessageBox_Option_Repository{
    ...
    protected function get_store(){
        ...
        if(!Gc_MessageBox_Service_Locator::has("multi_option_store")){
            throw new Exception("No option store engine");
        }
        $this->store = Gc_MessageBox_Service_Locator::get("multi_option_store");
        return $this->store;
    }
    ...
    protected function create_namespace_instance(){
        return Gc_MessageBox_CF::create("Option_Namespace");
