<?php
if(class_exists('Gc_MessageBox_Option_Group')) {
    return;
}
class Gc_MessageBox_Option_Group{
    protected $title = "";
    protected $id = "";
    ...
    protected function load_sub_groups_from_array($sub_groups){
        foreach($sub_groups as $key => $descriptor){
            $this->sub_groups[$key] = Gc_MessageBox_CF::create_and_init("Option_Group", $descriptor);
        }
    }
