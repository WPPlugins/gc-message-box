<?php
if(class_exists('Gc_MessageBox_Setting_Repository')) {
    return;
}

class Gc_MessageBox_Setting_Repository
implements
    ...
    Gc_MessageBox_Serializable_Interface
{
    ...
    protected function create_resultset($config,$value,$instance = null){
        $result_params = Gc_MessageBox_CF::create_and_init(
            "Setting_Resultset", 
            array(
...
//        $result[] = $this->create_resultset($this->get_multi_params_config(),$this->multi_params[$id],$id);
    ...
    public function get_new_multi_params(){
        return Gc_MessageBox_CF::create_and_init(
            "Setting_Resultset", 
            array(
                'namespace'  => $this->get_namespace(),
    ...
    public function save(){
        ...
        $store->save_option(GC_MESSAGE_BOX_TYPE,$data);
    }

    public function load(){
        $store = $this->get_store_instance();
        $this->configure($store->get_option(GC_MESSAGE_BOX_TYPE,array()));

    }
    protected function get_store_instance(){
        if(defined(GC_MESSAGE_BOX_SL_SETTING_STORE)){
            return Gc_MessageBox_Service_Locator::get(GC_MESSAGE_BOX_SL_SETTING_STORE);            
        }
        return Gc_MessageBox_CF::create("Setting_Store");
    }
    /* #ENDREGION */
    /* #REGION $namespace */
    protected $namespace ="";
