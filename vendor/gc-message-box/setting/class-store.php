<?php
if(class_exists('Gc_MessageBox_Setting_Store')) {
    return;
}

class Gc_MessageBox_Setting_Store 
implements Gc_MessageBox_Option_Store_interface{
    ...
    public function get_store(){
        $store = Gc_MessageBox_Service_Locator::get($this->option_store_name);
        if(is_null($store)){
            $store = Gc_MessageBox_CF::create('Option_Store');
        }
        return $store;        
    }
