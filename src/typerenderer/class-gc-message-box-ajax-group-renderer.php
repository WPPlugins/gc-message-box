<?php
if(class_exists("Gc_MessageBox_Ajax_Group_Renderer")){
    return;
}

class Gc_MessageBox_Ajax_Group_Renderer extends Gc_MessageBox_Abstract_Renderer{
      ...
      <div class="item displayfilter <?php /* echo $this->get_descriptor_param("css_class"); */ ?>" <?php if(!$description->is_visible()){ echo 'style="display:none"';}?> id="<?php echo $description->get_name(); ?>_cnt">
			...
			<div class="pagelist" id="gc-message-box-specified-pages">
				<?php 
					...
					//If edit this, you need to rewrite Ajax handler!!!! -> src/class-gc-message-box-admin-controller.php -> create_specified_pages_element()
				?>
						<div class="pageitem" id="gc-message-box-specified-page-<?php echo $key;?>">
                            ...
                            <input type="hidden" id="gc-message-box-specified-page-key-<?php echo $key;?>" value="<?php echo $url;?>" data-url="<?php echo $url;?>"/>
		...
		jQuery(document).ready(function(){
			...
			jQuery('#<?php echo $description->get_name(); ?>_add').on('click', function() {
				...
				jQuery.ajax({
				   ...
				   dataType:'json'
					...
					if (response.success) {
						jQuery('#gc-message-box-specified-pages').append(response.result);
			...
			jQuery('#gc-message-box-specified-pages').on('click', '.specified-close-page', function() {
